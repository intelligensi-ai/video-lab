...
Video Lab revision audited: `6a9a65fcefe8dcb736e941c8f5bbbc8e50e839e6` on `main`
Deploy Studio revision audited: `5459d7789dd451087a8452e399ec11c8db9dddc9` on `agent/longform-ltx-enhancer`
...
2. The browser creates and edits its storyboard through Video Lab. A complete local IndexedDB copy preserves selected `File` objects, while a media-free private draft is also saved through `/api/v1/storyboards/draft` for restart recovery.
...
- Constrained storyboard drafts to an allow-list, six shots, 256 KiB and no embedded base64 media.
...
- Upgraded Vite, replaced the unnecessary Router 7 server/RSC surface with browser-only Router 6.30.4, and pinned patched transitive URL/XML parsers. `pnpm audit --prod --audit-level high` reports no high or critical advisories.
...
| Cross-user IDOR              | Every project, draft, upload, generation and download is resolved against the authenticated UID         | Requires Firebase emulator/deployed multi-user acceptance                                       |
...
| Dependency advisories        | Exact lockfile, frozen install, patched Vite/router/transitive parsers, high-severity audit gate         | Five moderate advisories remain: router server/SSR paths are unused and navigation targets are fixed/internal; UUID buffer APIs are not called directly. Track upstream fixes |
...
9. Continue the new route-level split beyond the 48 KiB LongForm chunk; the remaining Firebase/auth/account/admin bundle is still about 988 KiB minified and triggers Vite's 500 KiB warning.
10. Integrate the real server-side entitlement provider without trusting browser state. Payment work remains out of scope.
11. Replace the current one-draft-per-user persistence with an owner-scoped multi-project model, project history and explicit project/asset deletion before marketing project management as complete.
