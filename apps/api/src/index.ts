...
let runtimeConfigurationPromise:Promise<void>|undefined;
async function ensureRuntimeConfiguration(){if(localAuth||runtimeConfigurationPromise)return runtimeConfigurationPromise;runtimeConfigurationPromise=(async()=>{adminApp();const snapshot=await getFirestore().collection('runtimeState').doc('config').get();if(!snapshot.exists)return;const configured=snapshot.data()?.baseUrl;if(configured===null){runtimeBaseUrl=undefined;runtimeState={...runtimeState,status:'unavailable',acceptingSubmissions:false,killSwitch:false,updatedAt:nowIso()};return;}const baseUrl=normalizeRuntimeBaseUrl(configured);if(baseUrl){runtimeBaseUrl=baseUrl;runtime=createRuntimeAdapter(baseUrl);}})().catch(error=>{runtimeConfigurationPromise=undefined;log('runtime_configuration_load_failed',{error:error instanceof Error?error.message:String(error)});throw error;});return runtimeConfigurationPromise;}
...
async function persistRuntimeConfiguration(baseUrl:string|null,principal:Principal){if(localAuth)return;adminApp();await getFirestore().collection('runtimeState').doc('config').set({baseUrl,updatedAt:nowIso(),updatedBy:principal.uid},{merge:true});}
...
app.get('/v1/runtime/status',auth,async(_req,res,next)=>{try{await ensureRuntimeConfiguration();await refreshRuntimeHealth();res.json({...runtimeState,baseUrl:runtimeBaseUrl,queueDepth:queue.filter(q=>q.status!=='done').length});}catch(e){next(e)}});
app.post('/v1/admin/runtime/connect',auth,async(req,res,next)=>{try{ const p=res.locals.principal as Principal;const baseUrl=normalizeRuntimeBaseUrl(req.body?.lambdaIp??req.body?.baseUrl); if(!baseUrl) throw problem(400,'invalid_runtime_endpoint','Enter a valid Lambda public IP address or HTTP endpoint'); const adapter=createRuntimeAdapter(baseUrl); const health=await adapter.healthCheck(); if(!health.ok) throw problem(502,'runtime_health_failed',health.message??'Runtime health check failed'); await persistRuntimeConfiguration(baseUrl,p);runtimeConfigurationPromise=Promise.resolve();runtime=adapter; runtimeBaseUrl=baseUrl; runtimeState={...runtimeState,provider:health.provider,status:'healthy',acceptingSubmissions:true,killSwitch:false,lastHeartbeatAt:nowIso(),updatedAt:nowIso()}; log('testing_runtime_connected',{baseUrl,worker:health.worker,uid:p.uid}); res.json({...runtimeState,baseUrl,health}); }catch(e){next(e)}});
app.post('/v1/admin/runtime/release',auth,async(_req,res,next)=>{try{if(queue.some(item=>item.status!=='done'))throw problem(409,'runtime_busy','Wait for the active generation to finish before releasing the runtime');const p=res.locals.principal as Principal;await persistRuntimeConfiguration(null,p);runtimeConfigurationPromise=Promise.resolve();const previousBaseUrl=runtimeBaseUrl;runtimeBaseUrl=undefined;runtimeState={...runtimeState,status:'unavailable',acceptingSubmissions:false,killSwitch:false,lastHeartbeatAt:undefined,updatedAt:nowIso()};log('testing_runtime_released',{previousBaseUrl,uid:p.uid});res.json({...runtimeState,baseUrl:undefined});}catch(e){next(e)}});
