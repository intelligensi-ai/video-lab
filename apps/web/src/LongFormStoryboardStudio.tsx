...
const initialScenes: StoryboardScenePayload[] = [
  { id: 'scene-1', title: 'The signal appears', prompt: '', duration: 5, trimStart: 0, trimEnd: 5, seed: 1337, transition: 'cut', transitionDuration: 0.75, carryPreviousFrame: true },
  { id: 'scene-2', title: 'Across the bridge', prompt: '', duration: 5, trimStart: 0, trimEnd: 5, seed: 1338, transition: 'crossfade', transitionDuration: 0.75, carryPreviousFrame: true },
];
...
function Preview({ generation, loading, cancelling, onCancel, cancelError }: { generation?: Generation; loading: boolean; cancelling: boolean; onCancel: () => void; cancelError?: string }) { const video=useAuthenticatedVideo(generation?.output?.downloadUrl); return <section className="lf-preview lf-panel"><header><div><span className="lf-label">Your creation</span><h2>Cinematic preview</h2><p>Intelligensi.ai Storyboard Studio</p></div><span className="lf-complete">{loading ? 'Rendering' : generation?.status ?? 'Ready'}</span></header><div className="lf-preview-tabs"><span>ϟ <b>Create</b><small>Studio prompt</small></span><span>▣ <b>Render</b><small>LTX video</small></span><span>♢ <b>Privacy</b><small>Private session</small></span></div><div className="lf-screen">{video.objectUrl ? <video src={video.objectUrl} controls/> : generation?.output?.downloadUrl ? <div className="thumb big">Retrieving completed video…</div> : <img src="/images/longform-ltx-storyboard-studio-film-roll.webp" alt="Film roll containing a sequence of cinematic storyboard frames"/>}{loading && <div className="lf-rendering">Rendering storyboard film…</div>}</div>{loading && generation && <button type="button" className="lf-cancel" disabled={cancelling} onClick={onCancel}>{cancelling ? 'Cancelling…' : 'Cancel active render'}</button>}{cancelError && <p className="lf-error">{cancelError}</p>}{video.error && <p className="error">Video retrieval failed: {video.error}</p>}{video.objectUrl && <a className="lf-download" href={video.objectUrl} download={`${generation?.id ?? 'video'}.mp4`}>⇩ Download video</a>}<p className="lf-ready">{generation ? `Generated video ${generation.status} · ${generation.creditCost} credits` : 'Your generated film will appear here'}</p><div className="lf-stats"><span><b>⌁ Progress</b>{generation?.status === 'completed' ? '100%' : loading ? 'In queue' : '—'}</span><span><b>◷ Elapsed</b>Session</span><span><b>⚙ Est. cost</b>{generation ? `${generation.creditCost} credits` : '—'}</span></div><div className="lf-progress"/></section>; }
function History({ generations, onSelect }: { generations: Generation[]; onSelect: (generation: Generation) => void }) { return <section className="lf-history lf-panel"><header><span>▣</span><div><strong>Previous generated videos</strong><small>{generations.length} films in your recent gallery</small></div><b>Recent work</b></header>{generations.length > 0 ? <div className="lf-history-grid">{generations.map((generation) => <button key={generation.id} onClick={() => onSelect(generation)}><GenerationThumbnail generation={generation}/><strong>{generation.prompt}</strong><small>{generation.status} / {generation.creditCost} credits</small></button>)}</div> : <div className="lf-history-empty"><span>▶</span><div><strong>Your storyboard films will collect here</strong><small>Generate the first cut to begin your history.</small></div></div>}</section>; }
