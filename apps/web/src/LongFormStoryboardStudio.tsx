...
import { cancelGeneration, generateLongFormVideo, getGallery, getGeneration, getRuntimeStatus, processOne, type LongFormGenerationPayload, type ReferenceRole, type StoryboardScenePayload, type StoryboardTransition } from './api.js';
...
const initialForm: LongFormGenerationPayload = {
  ...
  enhancePrompt: true, postProcess: 'none', outputFormat: 'mp4',
...
export default function LongFormStoryboardStudio() {
  ...
  const workerKickStarted = useRef(new Set<string>());
  const workerKickInFlight = useRef(false);
  ...
  const kickWorker = () => {
    if (workerKickInFlight.current) return;
    workerKickInFlight.current = true;
    void processOne()
      .catch((error) => console.error('Generation worker request failed', error))
      .finally(() => {
        workerKickInFlight.current = false;
        void queryClient.invalidateQueries({ queryKey: ['gallery'] });
        void queryClient.invalidateQueries({ queryKey: ['generation', selected?.id] });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (generation) => {
      ...
      workerKickStarted.current.add(generation.id);
      kickWorker();
    },
  });
  useEffect(() => {
  ...
  useEffect(() => {
    if (!currentGeneration || !['queued', 'preparing'].includes(currentGeneration.status)) return;
    if (workerKickInFlight.current || workerKickStarted.current.has(currentGeneration.id)) return;
    workerKickStarted.current.add(currentGeneration.id);
    kickWorker();
  ...
  const plannedTotalFrames = useMemo(() => Math.max(0, Math.round(totalSeconds * form.fps)), [form.fps, totalSeconds]);
  ...
  const planScenes = () => {
    ...
    setForm((current) => {
      const count = Math.max(1, Math.min(MAX_STORYBOARD_SCENES, current.scenes.length || initialScenes.length));
      return { ...current, scenes: Array.from({ length: count }, (_, index) => ({ id: crypto.randomUUID(), title: beats[index] ?? `Scene ${index + 1}`, prompt: `${index === 0 ? 'Establish' : 'Continue from the previous final frame and develop'} the artistic goal: ${current.overallGoal} Scene ${index + 1} of ${count}; describe one clear action, camera beat, lighting progression, and final composition.`, duration: 5, trimStart: 0, trimEnd: 5, seed: DEFAULT_SCENE_SEED + index, transition: index === 0 ? 'cut' : 'crossfade', transitionDuration: .75, carryPreviousFrame: true })) };
    });
  };
  ...
  return <main className={`lf-page ${helpMode ? 'help-mode' : ''}`}>
    ...
    <div className="lf-layout"><div className="lf-controls">
      ...
      <section className="lf-panel lf-storyboard-settings"><span className="lf-label">Setup</span><h2>Storyboard settings</h2><div className="lf-settings"><Field label="Working resolution" help="Sets the frame dimensions. Draft sizes render faster; HD sizes contain more detail and require more processing."><select value={form.resolution} onChange={(event) => setForm((current) => ({ ...current, resolution: event.target.value }))}><option value="1024x576">Landscape Draft 1024x576</option><option value="1280x720">Landscape HD 1280x720</option><option value="576x1024">Phone Draft 576x1024</option><option value="720x1280">Phone HD 720x1280</option><option value="1080x1080">Square 1080x1080</option></select></Field><Field label="Frame rate" help="Frames shown per second. 24 fps feels cinematic; 25 fps suits PAL delivery; 30 fps feels slightly smoother."><select value={form.fps} onChange={(event) => setForm((current) => ({ ...current, fps: Number(event.target.value) }))}><option value={24}>24 fps</option><option value={25}>25 fps</option><option value={30}>30 fps</option></select></Field></div><div className="lf-plan"><button type="button" className="lf-outline" data-help="Refresh the current scene cards from the creative brief. Add or delete scene cards below to change the scene count." onClick={planScenes}>✣ Plan scenes</button><div className="lf-count" data-help="Shows the number of planned scenes and their combined running time."><strong>{form.scenes.length}/{MAX_STORYBOARD_SCENES}</strong> scenes<br/><strong>{Math.floor(totalSeconds / 60)}:{String(Math.round(totalSeconds % 60)).padStart(2, '0')}</strong> requested</div></div></section>
...
function UploadBox({ label, help, compact = false, file, onFile }: { label: string; help?: string; compact?: boolean; file?: File; onFile: (file?: File) => void }) {
  ...
  >
    <button type="button" className="lf-upload-drop" data-help={`${file ? 'Replace' : 'Add'} this ${label.toLowerCase()} image. Supported formats are PNG, JPEG and WebP.`} aria-label={`${file ? 'Replace' : 'Add'} ${label.toLowerCase()}`} onClick={() => inputRef.current?.click()}>
    ...
    {file && <button type="button" className="lf-upload-remove" data-help={`Remove this ${label.toLowerCase()} so the scene can render without that image anchor.`} aria-label={`Remove ${label.toLowerCase()}`} onClick={() => onFile(undefined)}>Remove</button>}
  </div>;
}
...
function Preview({ generation, loading, plannedTotalFrames, submissionError, canGenerate, generateLabel, onGenerate, onProcessQueue, cancelling, onCancel, cancelError }: { generation?: Generation; loading: boolean; plannedTotalFrames: number; submissionError?: string; canGenerate: boolean; generateLabel: string; onGenerate: () => void; onProcessQueue: () => void; cancelling: boolean; onCancel: () => void; cancelError?: string }) {
  ...
  const durationSeconds = Number(generation?.settings.durationSeconds ?? generation?.output?.durationSeconds ?? 0);
  const fps = Number(generation?.settings.fps ?? generation?.settings.frameRate ?? 24);
  ...
  const renderedFrames = totalFrames ? Math.min(totalFrames, Math.round((progress / 100) * totalFrames)) : 0;
  const statusLabel = generation?.status ? generation.status.replace('_', ' ') : 'Ready';
  const queueLabel = generation?.queuePosition ? `Queue position ${generation.queuePosition}` : 'Waiting for worker';
  const activityLabel = generation?.runtimeMessage || (generation?.status === 'queued' ? queueLabel : `${statusLabel} with runtime`);

  return <section className="lf-preview lf-panel"><header data-help="This panel tracks the currently selected generation, including active render progress, playback and download once complete."><div><span className="lf-label">Your creation</span><h2>Cinematic preview</h2><p>Intelligensi.ai Storyboard Studio</p></div><span className="lf-complete" data-help="The current state of the selected generation: ready, queued, rendering, completed, failed or cancelled.">{loading ? 'Rendering' : submissionError ? 'Action needed' : statusLabel}</span></header><div className="lf-preview-tabs"><span data-help="Your creative brief and individual scene directions form the generation plan.">ϟ <b>Create</b><small>Studio prompt</small></span><span data-help="LTX converts the planned scenes and frame anchors into moving video clips.">▣ <b>Render</b><small>LTX video</small></span><span data-help="Your generation session and resulting film remain associated with your private account.">♢ <b>Privacy</b><small>Private session</small></span></div><div className="lf-screen" data-help="The selected generated film appears here. While empty, this shows the Storyboard preview artwork.">{video.objectUrl ? <video src={video.objectUrl} controls/> : generation?.output?.downloadUrl ? <div className="thumb big">Retrieving completed video…</div> : <img src="/images/longform-ltx-storyboard-studio-film-roll.webp" alt="Film roll containing a sequence of cinematic storyboard frames"/>}{loading && <div className="lf-rendering" data-help="Live render feedback. The frame counter is estimated from total storyboard seconds, frame rate and generation progress."><div><strong>{progress ? `${progress}% rendered` : 'Starting render'}</strong><small>{activityLabel}</small>{totalFrames > 0 && <b>Frame {renderedFrames.toLocaleString()} / {totalFrames.toLocaleString()}</b>}</div></div>}{!loading && submissionError && <div className="lf-rendering lf-render-error" data-help="The generation request was rejected before a new render started. Finish or cancel the active render, then submit again."><div><strong>Generation is already running</strong><small>{submissionError}</small></div></div>}</div><div className="lf-preview-actions"><button type="button" data-help="Validate the brief and scenes, then submit the complete storyboard to the video-generation queue." disabled={!canGenerate} className="lf-primary lf-generate" onClick={onGenerate}>{generateLabel}</button>{loading && generation && <button type="button" className="lf-process-now" data-help="Force VideoLab to process the current generation queue if the render is still waiting for a worker." onClick={onProcessQueue}>Process queue now</button>}{loading && generation && <button type="button" className="lf-cancel" data-help="Ask the generation service to stop the currently active storyboard render." disabled={cancelling} onClick={onCancel}>{cancelling ? 'Cancelling…' : 'Cancel active render'}</button>}</div>{cancelError && <p className="lf-error">{cancelError}</p>}{video.error && <p className="error">Video retrieval failed: {video.error}</p>}{video.objectUrl && <a className="lf-download" data-help="Save the completed film file to your device." href={video.objectUrl} download={`${generation?.id ?? 'video'}.mp4`}>⇩ Download video</a>}<p className="lf-ready" data-help="Plain-language status for the selected generated film.">{generation ? `Generated video ${generation.status}` : 'Your generated film will appear here'}</p><div className="lf-stats"><span data-help="Shows whether the selected film is waiting, rendering or complete."><b>⌁ Progress</b>{generation ? `${progress}% ${statusLabel}` : '—'}</span><span data-help="Estimated frames rendered from the current runtime progress."><b>▥ Frames</b>{totalFrames ? `${renderedFrames}/${totalFrames}` : '—'}</span><span data-help="Time since this generation was submitted."><b>◷ Elapsed</b>{formatElapsed(generation?.createdAt, now)}</span></div><div className="lf-progress" data-help="Visual progress bar for the selected render." aria-label={`${progress}% render progress`}><span style={{ width: `${progress}%` }}/></div></section>;
}
