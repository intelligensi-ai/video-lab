...
export default function LongFormStoryboardStudio() {
  ...
  return <main className={`lf-page ${helpMode ? 'help-mode' : ''}`}>
    ...
    </div><aside className="lf-preview-col"><Preview generation={currentGeneration} loading={isRendering} plannedTotalFrames={plannedTotalFrames} submissionError={mutation.error?.message} canGenerate={!invalid && !mutation.isPending} generateLabel={mutation.isPending ? '◌ Rendering storyboard...' : 'ϟ Generate Storyboard Film'} onGenerate={() => mutation.mutate()} cancelling={cancellation.isPending} onCancel={() => cancellation.mutate()} cancelError={cancellation.error?.message}/><History generations={history} onSelect={setSelected}/></aside></div>
  </main>;
}
...
function Preview({ generation, loading, plannedTotalFrames, submissionError, canGenerate, generateLabel, onGenerate, cancelling, onCancel, cancelError }: { generation?: Generation; loading: boolean; plannedTotalFrames: number; submissionError?: string; canGenerate: boolean; generateLabel: string; onGenerate: () => void; cancelling: boolean; onCancel: () => void; cancelError?: string }) {
